...
const shortestCommonSupersequenceRec = (s1, s2, index1, index2) => {
  ...
  if (s1[index1] === s2[index2]) {
    ...
    const s1Fixes = shortestCommonSupersequenceRec(s1, s2, index1, index2 + 1)
    const s2Fixes = shortestCommonSupersequenceRec(s1, s2, index1 + 1, index2)
...
const shortestCommonSupersequenceTopDown = (s1, s2, index1, index2, dp) => {
  ...
  if (dp[index1][index2] === -1) {
    ...
    if (s1[index1] === s2[index2]) {
      ...
      const s1Fixes = shortestCommonSupersequenceTopDown(s1, s2, index1, index2 + 1, dp)
      const s2Fixes = shortestCommonSupersequenceTopDown(s1, s2, index1 + 1, index2, dp)
      if (s1Fixes.length <= s2Fixes.length) {
        min = s2[index2] + s1Fixes
      } else {
        min = s1[index1] + s2Fixes
...
const main = function () {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    const result = shortestCommonSupersequence(input[i][0], input[i][1])
