  ...
  for(let i =1 ; i <= rows; i++) {
    for (let j = 1; j <= columns; j++) {
      if(s1[i -1] === s2[j- 1]) {
        ...
        let prevCol  = dp[i-1][j] + s1[i-1].charCodeAt(0) // previous column  + current character at the s1
        let prevRow  = dp[i][j -1] + s2[j-1].charCodeAt(0) // previous row  + current character at the s2
...
var main = function () {
  ...
  for (var i = 0; i < input.length; i++) {
      ...
      var result = fn(input[i][0], input[i][1]);
