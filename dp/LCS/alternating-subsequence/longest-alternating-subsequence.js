...
const LASRec = (numbers, previous, current, isAscending) => {
  ...
  */
  ...
  const count2 = LASRec(numbers, previous, current + 1, isAscending)
...
const LASRecMemo = (numbers, previous, current, isAscending, dp) => {
  // base case
  if (current === numbers.length) return 0
  const hash = `${previous}-${current}-${isAscending ? 1 : -1}`
  if (dp.has(hash) === false) {
    ...
    // do not include
    const count2 = LASRecMemo(numbers, previous, current + 1, isAscending, dp)
