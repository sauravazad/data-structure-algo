...
function countRotations (arr) {
  ...
  while (low <= high) {
    ...
    const prev = (mid - 1 + n) % n; const next = (mid + 1) % n// if first element is mid or
    // last element is mid then simply use modulo so it never goes out of bound.
    if (arr[mid] <= arr[prev] && arr[mid] <= arr[next]) { return mid } else if (arr[mid] <= arr[high]) { high = mid - 1 } else if (arr[mid] >= arr[low]) { low = mid + 1 }
  }
  return 0
}
